"""
Holder Count Analysis Utilities for DeFi Analytics

This module provides functions for analyzing holder count data,
including growth rates, anomaly detection, and pattern classification.
"""
...
def analyze_holder_count_data(df: pd.DataFrame, holder_count_col: str = "holder_count") -> Dict:
    """Alias for calculate_holder_growth_metrics for backwards compatibility."""
    return calculate_holder_growth_metrics(df, holder_count_col)


def generate_holder_count_summary(metrics: Dict) -> str:
    """
    Generate a text summary from holder count metrics.

    Args:
        metrics: Dictionary from calculate_holder_growth_metrics

    Returns:
        Summary text string
    """
    pattern = classify_growth_pattern(metrics)

    return (
        f"Holder Count Analysis:\n"
        f"  - Start: {metrics['start_holders']:,} holders\n"
        f"  - Current: {metrics['end_holders']:,} holders\n"
        f"  - Total Growth: {metrics['total_growth']:+,} ({metrics['total_growth_rate']:+.2f}%)\n"
        f"  - Avg Daily Growth: {metrics['avg_daily_growth']:+.1f} ({metrics['avg_daily_growth_rate']:+.2f}%)\n"
